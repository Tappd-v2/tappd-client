...
<template>
   <div
      :class="[
         showDetails ? 'h-auto border-t border-gray-200 p-4' : 'h-0', // h-0 instead of hidden for animation
      ]"
      class="order-details"
   >
      <div
      ...
      >
         <p class="flex-grow text-lg" @click="selectItem(item)">
            {{ item.amount }}x {{ item.name }}
         </p>
         <button class="text-gray-500" @click.prevent="removeItem(item.id)">
...
.order-details > div.removing {
   /* New class for removing items */
